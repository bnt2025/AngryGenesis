...
class CC2531(object):
    ...
    def _set_ctrl(self, c=197, i=4):
        if c not in self.CTRL_LEN:
            ...
            l = self.CTRL_LEN[c]
        ret = self.com.controlWrite(0x40, c, 0, i, l*'\0')
        if self.DEBUG > 2:
            self._log('(_set_ctrl, c %i, i %i) ret: %i' % (c, i, ret))

    def _set_chan(self, chan=0x0b):
        ret = self.com.controlWrite(
            0x40, 210, 0, 0, chr(min(255, max(0, chan))))
        if self.DEBUG > 2:
    ...
    def read_data(self):
        ...
        try:
            ret = self.com.bulkRead(
                self.DATA_EP, self.DATA_BUFLEN, self.READ_TO)
        except libusb1.USBError:
            ret = ''
        if self.DEBUG > 1:
